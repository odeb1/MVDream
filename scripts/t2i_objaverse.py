...
if __name__ == "__main__":
    ...
    parser.add_argument("--text", type=str, default="teddy_bear")
    ...
    parser.add_argument("--laion_data_ordinal", type=int, default=28)
    ...
    uc = model.get_learned_conditioning( ["panda"] ).to(device)
    ...
    lvis_class_of_interest = args.text
    prompts = f'/home/ubuntu/code/MVDream-threestudio/extern/objaverse/prompts_per_lvis_class/{lvis_class_of_interest}.csv'
    df = pd.read_csv(prompts, engine='python')
    outfolder = f'objaverse/{lvis_class_of_interest}'
    ...
    for i in range(len(df)):
        ...
        Image.fromarray(images).save(f"{outfolder}/{uid}.png")